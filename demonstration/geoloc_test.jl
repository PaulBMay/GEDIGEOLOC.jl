...
priors = (α = [0.0, 100.0], τ = gammashaperate(1/epsilon, 10), Σ = [50.0, diagm(fill(100.0, 2))], ϕ = gammashaperate(rangeOffset, 1_000))

initial = (w = zeros(n,2), Σ = diagm(fill(100.0, 2)), α = 0.0, ϕ = rangeOffset, ρ = 1 - noiseOffset, μ = muOffset, τ = 1/epsilon)

samples = geolocate(y, loc, grid, refVals, priors, initial, 10)
samples = geolocate(y, loc, grid, refVals, priors, initial, 10000)
